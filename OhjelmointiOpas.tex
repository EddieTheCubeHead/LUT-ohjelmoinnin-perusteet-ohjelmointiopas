...
\subsection{Luokkien käyttötarkoitus}

\newthought{Luokat ovat} Javan ydin ja kaiken koodin rakennuspala. Kaikki koodi Javassa on
suljettava luokan sisään. Ohjelmistot koostuvat pääluokasta, joka sisältää main-metodin. Tämä
metodi määritellään avainsanoilla \java{public} \java{static} \java{void}. Void-avainsana
selitetään myöhemmin kappaleessa ja static-avainsana kappaleessa *x*. $ TODO: lisää kappale $
Toistaiseksi riittää sisällyttää nämä avainsanat annetussa järjestyksessä main-metodin
määritelmään.

\subsection{Näkyvyysmääreet}

Muuttujan luominen Javassa tukee tietotyypin määrittelyn lisäksi muuttujan näkyvyyden
määrittämistä. Tämä määritelmä on nimeltään \neweng{nakyvyysmaare}{access modifier}. Kuten
oppaassa aiemmin todettiin, yksi olio-ohjelmoinnin ydinideoista on \new{enkapsulaatio}, eli
koodin osien vastuiden tarkka rajaaminen. Näkyvyysmääreet ovat tärkein työkalu tämän suhteen.
...
\midrule
\newjava{private} & Vain tämä luokka \\
\addlinespace
...
\addlinespace
\newjava{public} & Kaikki luokat kaikkialla \\
\bottomrule
\end{tabular}
\end{center}
